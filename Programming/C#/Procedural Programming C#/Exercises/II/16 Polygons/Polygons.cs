...
namespace ConsoleApp
{
    class Program
    {
        ...
        static bool IsPolyEqConditions(Polygon p) // Прверяем многоугольник на соответствие зад
        {
        ...
        static Polygon Constructor(int maxSideLen) // Функция для создания одного многоугольника
        {
            string[] colors = { "Red", "White", "Black", "Yellow", "Blue", "Green", "Purple" };
            int sides = rnd.Next(3, 7); // Случайное количество сторон
            int w = rnd.Next(6); // Случайная толщина линии
            string c = colors[rnd.Next(colors.Length)]; // Случайный цвет
            bool f = 1 > rnd.Next(2) ? false : true; // Случайная заливка фигуры

            Polygon obj = new Polygon(w, c, f);

            for (int i = 0; i < sides; i++)
            {
                // Случайные координаты в пределах заданного значения са округлением до 2 знаков после запятой
                double one = Math.Round(rnd.NextDouble() * (maxSideLen * 2) - maxSideLen, 2);
                double two = Math.Round(rnd.NextDouble() * (maxSideLen * 2) - maxSideLen, 2);
                while (one == two)
                {
                    two = Math.Round(rnd.NextDouble() * (maxSideLen * 2) - maxSideLen, 2);
                }
